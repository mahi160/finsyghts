...
export const AllCurrencies: React.FC<{ selected: Array<ICurrency> }> = ({
  ...
  const renderCurrencyItem = (
    ...
    <Label
      ...
      className={cn(
        'flex items-center gap-2 p-2 hover:bg-secondary/50 rounded-md transition-colors',
    ...
    >
      ...
      />
      <div className="flex-1 text-xs">{currency.name}</div>
  ...
  return (
    <ResponsiveModal
      trigger={
        <Button
          variant="outline"
          className="h-10 w-full border-dashed hover:bg-secondary/50 transition-colors"
        >
          <Plus size={14} className="mr-2" />
          <span className="text-xs">Add Currency</span>
        </Button>
      }
